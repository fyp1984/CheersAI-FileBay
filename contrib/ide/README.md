...
Create new directory `.vscode` in the repository root folder and copy contents of folder [contrib/ide/vscode](vscode/) to it. You can now use `Ctrl`+`Shift`+`B` to build the server executable (default: `gitea`) and `F5` to run it in debug mode.
