...
该编排在一台受控主机上运行 FileBay、合成脱敏 MySQL 数据源、官方 RAGFlow 与官方 Dify。浏览器只访问 FileBay 的一个前端端口：FileBay 使用 `/knowledge`，RAGFlow 使用 `/ragflow/`。二者是独立后端服务，但在同一域名、端口与统一导航下运行；FileBay 负责企业知识治理，RAGFlow 保留模型、数据集和检索配置。
...
4. RAGFlow 配置工作区为同一前端端口下的 `http://localhost:13080/ragflow/`。该页面保留上游的模型供应商、数据集、解析和检索测试功能，顶部会显示“返回知识库工作台”，可随时回到 FileBay；FileBay 的“检索引擎”也提供反向入口。
