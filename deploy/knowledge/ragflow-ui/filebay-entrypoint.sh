...
if [[ -f "${index_file}" ]]; then
	if ! grep -q 'filebay-shell.js' "${index_file}"; then
		sed -i 's#</head>#<link rel="stylesheet" href="/ragflow/filebay-shell.css?v=20260725"><script src="/ragflow/filebay-shell.js?v=20260725"></script></head>#' "${index_file}"
	else
		...
		sed -i 's#filebay-shell\.css[^" ]*#filebay-shell.css?v=20260725#g; s#filebay-shell\.js[^" ]*#filebay-shell.js?v=20260725#g' "${index_file}"
	fi
fi
