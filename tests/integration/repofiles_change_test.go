...
func getExpectedFileResponseForRepoFilesCreate(commitID string, lastCommit *git.Commit) *api.FileResponse {
	...
	return &api.FileResponse{
		Content: &api.ContentsResponse{
			...
			LastCommitSHA:     ptr(lastCommit.ID.String()),
			LastCommitterDate: ptr(lastCommit.Committer.When),
			LastAuthorDate:    ptr(lastCommit.Author.When),
			Type:              "file",
			Size:              18,
			Encoding:          &encoding,
			Content:           &content,
...
func getExpectedFileResponseForRepoFilesUpdate(commitID, filename, lastCommitSHA string, lastCommitterWhen, lastAuthorWhen time.Time) *api.FileResponse {
	...
	return &api.FileResponse{
		Content: &api.ContentsResponse{
			...
			LastCommitterDate: ptr(lastCommitterWhen),
			LastAuthorDate:    ptr(lastAuthorWhen),
			Type:              "file",
			Size:              43,
			Encoding:          &encoding,
			Content:           &content,
...
func getExpectedFileResponseForRepoFilesUpdateRename(commitID, lastCommitSHA string) *api.FilesResponse {
	details := []struct {
		...
		},
		{
			...
			lfsOid:   ptr("2eccdb43825d2a49d99d542daa20075cff1d97d9d2349a8977efe9c03661737c"),
			lfsSize:  ptr(int64(2048)),
		},
		{
			filename: "jpeg.jpeg",
		...
		},
		{
			...
			lfsOid:   ptr("7b6b2c88dba9f760a1a58469b67fee2b698ef7e9399c4ca4f34a14ccbe39f623"),
			lfsSize:  ptr(int64(27)),
		},
	}
