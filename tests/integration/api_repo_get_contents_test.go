...
func getExpectedContentsResponseForContents(ref, refType, lastCommitSHA string) *api.ContentsResponse {
	...
	return &api.ContentsResponse{
		...
		LastCommitSHA:     ptr(lastCommitSHA),
		LastCommitterDate: ptr(time.Date(2017, time.March, 19, 16, 47, 59, 0, time.FixedZone("", -14400))),
		LastAuthorDate:    ptr(time.Date(2017, time.March, 19, 16, 47, 59, 0, time.FixedZone("", -14400))),
		Type:              "file",
		Size:              30,
		Encoding:          ptr("base64"),
		Content:           ptr("IyByZXBvMQoKRGVzY3JpcHRpb24gZm9yIHJlcG8x"),
		URL:               &selfURL,
		HTMLURL:           &htmlURL,
		GitURL:            &gitURL,
		DownloadURL:       ptr(setting.AppURL + "user2/repo1/raw/" + refType + "/" + ref + "/" + treePath),
...
func testAPIGetContentsExt(t *testing.T) {
	...
	t.Run("DirContents", func(t *testing.T) {
		...
		assert.Equal(t, ptr(int64(107)), respFile.LfsSize)
		assert.Equal(t, ptr("0b8d8b5f15046343fd32f451df93acc2bdd9e6373be478b968e4cad6b6647351"), respFile.LfsOid)
	})
