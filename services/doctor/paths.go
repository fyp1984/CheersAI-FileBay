...
func checkConfigurationFile(logger log.Logger, autofix bool, fileOpts configurationFile) error {
	...
	if err != nil {
		if os.IsNotExist(err) && autofix && fileOpts.IsDirectory {
			if err := os.MkdirAll(fileOpts.Path, 0o777); err != nil {
				...
				return fmt.Errorf("configuration directory %q does not exist and could not be created: %w", fileOpts.Path, err)
			}
			fi, err = os.Stat(fileOpts.Path)
		}
	}
	if err != nil {
		if fileOpts.Required {
			...
			return fmt.Errorf("configuration file %q is not accessible but is required: %w", fileOpts.Path, err)
		}
	...
	if fileOpts.IsDirectory && !fi.IsDir() {
		...
		return fmt.Errorf("configuration directory %q is not a directory: %w", fileOpts.Path, err)
	} else if !fileOpts.IsDirectory && !fi.Mode().IsRegular() {
		logger.Error("    ERROR: not a regular file")
		return fmt.Errorf("configuration file %q is not a regular file: %w", fileOpts.Path, err)
		...
		if err := isWritableDir(fileOpts.Path); err != nil {
			...
			return fmt.Errorf("configuration file %q is required to be writable but is not: %w", fileOpts.Path, err)
		}
	}
	return nil
}
